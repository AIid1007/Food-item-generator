...
(function () {
  ...
  function renderCell(dateStr, slot, opts = {}) {
    ...
    return `
      <div class="${classes.join(' ')}" data-date="${dateStr}" data-slot="${slot.id}">
        ${jharokhaArch}
        ${dish ? `
          <div>
    ...
    return `
      <div class="stack stack--lg">
        ...
        <div class="row row--wrap" style="margin-top:12px">
          <p class="muted tiny">Tap any cell to swap, lock, or mark cooked. Hover for more options.</p>
        </div>
      </div>
    `;
  }
      ...
      <div class="stack stack--lg">
        <div class="plan-header">
          <div class="plan-title-block">
            ...
            <h1 class="h1">Day overrides</h1>
            <p class="muted">Mark fasting days, vegan days, or festive days for the upcoming month. Generation will respect them.</p>
          </div>
        </div>
              ...
              <div class="dish-card" data-cal-date="${dStr}" style="cursor:pointer">
                <div class="row" style="justify-content:space-between">
                  <div>
                    ...
                    <div class="dish-card-meta">
                      ${ov ? `<span class="tag tag--vrat">${ov.mode}</span>` : '<span class="tag">no override</span>'}
                    </div>
                  </div>
  ...
  function boot() {
    ...
    if (state.library && state.library.length > 0) {
      const seedById = new Map(window.SEED_LIBRARY.map(d => [d.id, d]));
